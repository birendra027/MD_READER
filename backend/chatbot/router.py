...
async def list_session_files(session_id: str):
    """Return all files generated by this session with backend proxy download URLs."""
    ...
    # Always use the backend proxy URL — never expose S3/LocalStack URLs to browsers
    for item in items:
        item["url"] = _local_file_download_url(session_id, item["filename"])
    return {"files": items}


@router.get("/files/{session_id}/{filename}/url")
async def get_file_url(session_id: str, filename: str):
    """Return the backend proxy download URL for a specific session file."""
    if not _SESSION_RE.match(session_id):
        return {"error": "Invalid session"}
    safe_name = Path(filename).name
    key = s3_client._output_key(session_id, safe_name)
    # Check the file exists either in S3 or on local disk
    loop = asyncio.get_event_loop()
    s3_items = await loop.run_in_executor(None, s3_client.list_session_files, session_id)
    in_s3 = any(item["key"] == key for item in s3_items)
    if in_s3:
        return {
...
async def download_session_file(session_id: str, filename: str):
    """Serve a session file — try local disk first, then stream from S3."""
    from fastapi.responses import Response
    if not _SESSION_RE.match(session_id):
        return {"error": "Invalid session"}
    safe_name = Path(filename).name
    # 1. Prefer the locally written copy (always present when running in Docker)
    session_dir = _session_output_dir(session_id)
    ...
    # 2. Fall back to streaming the object from S3
    key = s3_client._output_key(session_id, safe_name)
    loop = asyncio.get_event_loop()
    result = await loop.run_in_executor(None, s3_client.download_fileobj, key)
